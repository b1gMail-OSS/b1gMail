<form action="{sessionurl file='plugin.page.php' params="plugin={$bmsPlugin}&do=smtp&action=greylist"}" method="post" onsubmit="spin(this)" name="f1">
	{csrffield}
	<input type="hidden" name="page" id="page" value="{$pageNo}" />
	
	<fieldset class="mb-4">
		<legend class="h4 mb-3">{lng p="bms_greylist"}</legend>
		
		<div class="card mb-3"><div class="table-responsive"><table class="table table-vcenter table-striped card-table">
			<tr>
				<th>{lng p="bms_ip"}</th>
				<th class="text-nowrap">{lng p="bms_grey_date"}</th>
			...
			<tr>
				...
				{/if}
				<td class="text-nowrap">{date timestamp=$item.time}</td>
				<td class="text-center"><label class="form-check justify-content-center mb-0"><input class="form-check-input" type="checkbox" disabled="disabled"{if $item.confirmed} checked="checked"{/if} /></label></td>
