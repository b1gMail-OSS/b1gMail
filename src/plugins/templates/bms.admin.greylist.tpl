		...
		<div class="card">
			<div class="table-responsive">
				<table class="table table-vcenter table-striped">
					...
					<tr>
						<th>{lng p="bms_ip"}</th>
						<th style="width: 160px;">{lng p="bms_grey_date"}</th>
						<th style="width: 100px;">{lng p="bms_grey_confirmed"}</th>
					...
					{foreach from=$greylist item=item key=key}
						...
						<tr class="{$class}">
							...
							<td><input type="checkbox" class="form-check-input" disabled="disabled"{if $item.confirmed} checked="checked"{/if} /></td>
