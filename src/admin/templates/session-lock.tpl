		...
		<div class="container-tight py-4">
			<form class="card card-md" id="bmSessionLockOverlayForm" method="post" autocomplete="off">
				...
				<div class="card-body text-center">
					...
					{if $_adminDisplayName|default:'' != '' || $adminRow.username|default:'' != ''}
					<div class="mb-4">
						{include file="user-avatar.tpl" avatarSize="xl" avatarClass="mb-3" avatarBgPrimary=true}
						<h3 class="mb-0">
							{if $_adminDisplayName|default:'' != ''}
							{text value=$_adminDisplayName allowEmpty=true}
							{else}
							{text value=$adminRow.username}
							{/if}
						</h3>
						{if $_adminDisplayName|default:'' != '' && $adminRow.username|default:'' != '' && $_adminDisplayName != $adminRow.username}
						<div class="text-secondary">{text value=$adminRow.username}</div>
						{elseif $adminRow.email|default:'' != ''}
						<div class="text-secondary">{email value=$adminRow.email}</div>
						{/if}
					</div>
					{/if}
					...
					<div class="d-grid gap-2">
						...
						<a href="{sessionurl file='index.php' params='action=logout'}" class="btn btn-ghost-secondary" onclick="return confirm('{lng p="logoutquestion"}');">{lng p="logout"}</a>
					</div>
				</div>
			</form>
		</div>
		...
		<div class="modal-content">
			...
			<div class="modal-footer">
				<div class="w-100">
					<div class="row">
						<div class="col">
							<a href="{sessionurl file='index.php' params='action=logout'}" class="btn w-100" onclick="return confirm('{lng p="logoutquestion"}');">{lng p="logout"}</a>
						</div>
						<div class="col">
