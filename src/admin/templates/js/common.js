...
function _checkForPluginUpdates(obj)
{
    if(obj.readyState == 4)
    {
        ...
        if(text.length == 4)
        {
            ...
            if(resultCode == 0)
            {
                EBID(layerName).innerHTML = '<i class="fa-solid fa-circle-question text-muted"></i> <p class="text-muted">(' + lang['unknown'] + ')</p>';
            }
            else if(resultCode == 1)
            {
                EBID(layerName).innerHTML = '<i class="fa-solid fa-circle-check text-green"></i> '
                    + lang['version'] + ': ' + text[2];
            }
            else if(resultCode == 2)
            {
                EBID(layerName).innerHTML = '<i class="fa-solid fa-triangle-exclamation text-warning"></i> '
