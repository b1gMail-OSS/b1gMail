	...
	<div class="mb-3 row">
		...
		<div class="col-sm-10">
			<div class="form-control-plaintext">
				<a href="{sessionurl file='users.php' params="do=edit&id={$payment.user.id}"}">{$payment.user.email}</a>
...
<div class="row">
	...
	<div class="col-md-4 text-center">{if $payment.hasInvoice}<input class="btn btn-muted" type="button" value="{lng p="invoice"}" onclick="openWindow('payments.php?action=showInvoice&orderID={$payment.orderid}{$sessionUrlSuffix}','invoice_{$payment.orderid}',640,480);" />{/if}</div>
	<div class="col-md-4 text-end">{if $payment.status==0}<input class="btn btn-primary" type="button" value="{lng p="activate"}" onclick="document.location.href='payments.php?singleAction=activate&singleID={$payment.orderid}{$sessionUrlSuffix}';" />{/if}</div>
</div>