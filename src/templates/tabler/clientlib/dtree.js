...
dTree.prototype.nodeStatus = function(status, id, bottom) {
	...
	if (this.config.useIcons) {
		...
		if (eIcon) {
			var iconClass = (status) ? this.aNodes[id].iconOpen : this.aNodes[id].icon;

			if (eIcon.tagName == 'IMG')
				eIcon.src = iconClass;
			else
				eIcon.className = iconClass;
		}

	}

	if (eJoin) {
		var joinClass = (this.config.useLines)?

			((status)?((bottom)?this.icon.minusBottom:this.icon.minus):((bottom)?this.icon.plusBottom:this.icon.plus)):

			((status)?this.icon.nlMinus:this.icon.nlPlus);

		if (eJoin.tagName == 'IMG')
			eJoin.src = joinClass;
		else
			eJoin.className = joinClass;
	}
