...
function readMailUpdateBottomLayerButtons(activeName)
{
	var footer = document.querySelector('#contentFooter.bm-mail-read-footer');
	if(!footer)
		return;

	var buttons = footer.querySelectorAll('[data-bottom-layer]');
	for(var i=0; i<buttons.length; i++)
	{
		var layerName = buttons[i].getAttribute('data-bottom-layer');
		if(activeName && layerName == activeName)
...
function readMailHideBottomLayers()
{
	...
	var scrollContainer = EBID('mailReadScrollContainer');
	if(scrollContainer)
		scrollContainer.setAttribute('class', 'scrollContainer withBottomBar bm-mail-read-scroll');

	var mainArea = EBID('mainContentArea');
	if(mainArea)
		mainArea.classList.remove('bm-mail-read-bottom-layer-open');

	readMailUpdateBottomLayerButtons(null);
}

function readMailShowBottomLayer(name)
{
	var layer = EBID('bottomLayer_'+name);
	var justClose = layer && layer.style.display != 'none' && layer.style.display != '';
	...
	if(!justClose && layer)
	{
		...
		var mainArea = EBID('mainContentArea');
		if(mainArea)
			mainArea.classList.add('bm-mail-read-bottom-layer-open');

		readMailUpdateBottomLayerButtons(name);
	}
}
...
function initMailSel()
{
	...
	{
		var mailID = this.getItemID(element);
		...
		if(EBID('previewArea'))
			togglePreviewPane(mailID, tplDir, currentSID);
	}
...
function composeSizer(justFire)
{
	var or = function()
	{
		var mainContent = EBID('mainContent'),
			composeHeader = EBID('composeHeader'),
			contentHeader = EBID('contentHeader'),
			contentFooter = EBID('contentFooter'),
			composeText = EBID('composeText');

		if(!mainContent || !composeHeader || !contentHeader || !contentFooter || !composeText)
			return;

		var height = getElementMetrics(mainContent, 'h') - 1;
		if(EBID('draftNote') && EBID('draftNote').style.display != 'none')
			height -= getElementMetrics(EBID('draftNote'), 'h');
		height -= getElementMetrics(composeHeader, 'h');
		height -= getElementMetrics(contentHeader, 'h');
		height -= getElementMetrics(contentFooter, 'h');
...
function bmIsBlankIframeUrl(iframeDoc)
{
	try
	{
		var href = iframeDoc.location.href;
...
function bmApplyMailIframeTheme(iframeDoc)
{
	if(!iframeDoc || !iframeDoc.documentElement)
		return;

	if(!iframeDoc.getElementById('bmMailIframeTheme'))
	{
		var style = iframeDoc.createElement('style');
		style.id = 'bmMailIframeTheme';
		style.textContent = 'html, body { background: #fff !important; color: #333 !important; color-scheme: light !important; margin: 0; padding: 0; } '
			+ '#__bmMailText { background: #fff !important; color: #333; padding: 1rem 1.25rem; box-sizing: border-box; } '
			+ 'a { color: ' + (typeof bmPrimaryColor === 'string' && bmPrimaryColor ? bmPrimaryColor : '#066fd1') + '; }';
...
function bmBindMailIframeDoc(iframe)
{
	...
	var resCB = function()
	{
		...
		else if(iframeDoc && iframeDoc.documentElement)
		{
		...
		if(http.readyState == 4)
		{
			...
			for(var i=0; i<id.length; ++i)
			{
				...
				else if(flag == 16)
				{
					...
					if(img && typeof(img.src) != 'undefined' && img.src)
					{
					...
					{
						img.className = results[i] == '1' ? 'ti ti-flag-filled bm-mail-flag-icon' : 'bm-mail-flag-placeholder';
					}
				}
...
function _folderColorMail(http)
{
	if(http.readyState == 4)
	{
		...
		if(tr)
		{
			tr.className = tr.className.replace(/\s*mailColor_\d+/g, '');
			if(color > 0)
				tr.className += ' mailColor_' + color;
		}
	}
}
