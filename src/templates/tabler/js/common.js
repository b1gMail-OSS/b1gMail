...
{
	initBmTheme();
	initBmNavbarDropdowns();
	bmSyncTablerLayout();
	addEvent(window, 'resize', bmSyncTablerLayout);
...
var BM_THEME_KEY = 'bm-tabler-theme';

function bmGetTheme()
{
	try {
		var t = localStorage.getItem(BM_THEME_KEY);
		if(t === 'dark' || t === 'light')
			return t;
	} catch(e) {}
	return null;
...
function bmSetTheme(theme)
{
	if(theme !== 'dark' && theme !== 'light')
		return;

	try {
		localStorage.setItem(BM_THEME_KEY, theme);
	} catch(e) {}
...
function initBmTheme()
{
	if(typeof(bmEnableDarkMode) === 'undefined' || !bmEnableDarkMode)
		return;

	var theme = bmGetTheme();
	if(theme)
		bmSetTheme(theme);
}

function initBmNavbarDropdowns()
{
	var bs = (typeof bootstrap !== 'undefined' && bootstrap)
		|| (typeof tabler !== 'undefined' && tabler.bootstrap)
		|| (typeof window.tabler !== 'undefined' && window.tabler.bootstrap);
	if(!bs || !bs.Dropdown)
		return;
	...
	document.querySelectorAll('[data-bs-toggle="dropdown"]').forEach(function(el) {
		if(el.dataset.bmDropdownInit === '1')
			return;
		el.dataset.bmDropdownInit = '1';
		bs.Dropdown.getOrCreateInstance(el);
	});
}

function bmIsTablerMobile()
{
	return window.matchMedia('(max-width: 991.98px)').matches;
}

function bmIsLegacyAddonPage()
{
	var area = EBID('mainContentArea');
	if(!area)
		return false;

	if(area.querySelector('.bm-dashboard-page, .bm-organizer-page, .bm-organizer-header, .bm-compose-header, .bm-compose-fields, .bm-mail-read-page-header, .bm-folder-admin-header, #folderMailArea, #mailTable, #mailReadScrollContainer'))
		return false;

	/* Migrierte Prefs-Seiten – Legacy-Plugin-Prefs (nur contentHeader + scrollContainer) ausnehmen */
	...
	var header = area.querySelector('#contentHeader, .contentHeader');
	if(header)
	{
	...
	var footer = area.querySelector('#contentFooter, .contentFooter');
	if(footer)
		return true;

	/* Plugin-Prefs / Info-Seiten: Header + Scroll-Bereich ohne Footer */
	return !!header;
}

function bmSyncLegacyAddonLayout()
{
	var area = EBID('mainContentArea');
	var isLegacy = bmIsLegacyAddonPage();

	if(area)
		area.classList.toggle('bm-legacy-addon', isLegacy);
...
function advancedOptions(field, dir1, dir2, tpldir)
{
	...
	if(body.style.display=='')
	{
		...
		else if(arrow.classList && arrow.classList.contains('ti'))
		{
			arrow.classList.remove('ti-chevron-right', 'ti-chevron-down', 'ti-chevron-left', 'ti-chevron-up');
			arrow.classList.add('ti-chevron-' + dir1);
		}
	...
	else
	{
		...
		{
			arrow.classList.remove('ti-chevron-right', 'ti-chevron-down', 'ti-chevron-left', 'ti-chevron-up');
			arrow.classList.add('ti-chevron-' + dir2);
		}
