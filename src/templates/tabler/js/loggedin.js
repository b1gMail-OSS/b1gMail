...
function setNotificationCount(count)
{
	if(count > 0)
	{
		...
		if(newNotifications)
		{
			var notifyIcon = EBID('notifyIcon');
			notifyIcon.className = notifyIcon.className.replace(/\s*(animated|faa-ring|icon-pulse)\s*/g, ' ').trim() + ' icon-pulse';
	...
	else
	{
		...
		EBID('notifyIcon').className = EBID('notifyIcon').className.replace(/\s*(animated|faa-ring|icon-pulse)\s*/g, ' ').trim();
	}
...
function _previewNote(request)
{
	...
	{
		var preview = EBID('notePreview');
		if(preview)
		{
			preview.classList.remove('bm-organizer-note-preview-empty');
			preview.innerHTML = NL2BR(HTMLEntities(request.responseText));
		}
	}
}
...
function initVSep()
{
	var sepParent = EBID('mainContentArea') || EBID('mainContent');
	var vSep1 = EBID('vSep1'), vSep2 = EBID('vSep2'), vSepSep = EBID('vSepSep');

	vSepSep.onmousedown = function(event) { if(typeof(bmIsTablerMobile) != 'undefined' && bmIsTablerMobile()) return false; if(!event&&window.event) event=window.event; _vSepDragging = true; _vSepStartY = event.clientY - getElementMetrics(vSepSep, 'y'); return(false); };

 	var omu = function() { _vSepDragging = false; if(typeof(bmIsTablerMobile) == 'undefined' || !bmIsTablerMobile()) setCookie('vSepHeight1', parseInt(vSep1.style.height)); if(typeof(IE7) != 'undefined') IE7.recalc(); };
 	...
 	var omm = function(event)
 	{
		...
		if(_vSepDragging && (typeof(bmIsTablerMobile) == 'undefined' || !bmIsTablerMobile()))
		{
	...
	var or = function(event)
	{
		...
		{
			vSep1.style.cssText = vSep2.style.cssText = vSepSep.style.cssText = '';
			return;
		}
...
function _hSepUsesFlex(sepParent)
{
	if(!sepParent || typeof(window.getComputedStyle) == 'undefined')
		return(false);

	var display = window.getComputedStyle(sepParent).display;
	return(display == 'flex' || display == 'inline-flex');
}

function _hSepApplyWidths(hSep1, hSep2, hSepSep, sepParent, width1, sepWidth)
{
	if(_hSepUsesFlex(sepParent))
	{
		hSep1.style.flex = '0 0 ' + width1 + 'px';
		hSep1.style.width = width1 + 'px';
		hSep2.style.flex = '1 1 auto';
		hSep2.style.width = '';
		hSep2.style.left = '';
		if(hSepSep) hSepSep.style.left = '';
	}
	else
	{
		...
		if(hSepSep) hSepSep.style.left = width1 + 'px';
	}
}
...
function initHSep(instName)
{
	...
	hSepSep.onmousedown = function(event) { if(typeof(bmIsTablerMobile) != 'undefined' && bmIsTablerMobile()) return false; if(!event&&window.event) event=window.event; _hSepDragging = true; _hSepStartX = event.clientX - getElementMetrics(hSepSep, 'x'); return(false); };

 	var omu = function() { _hSepDragging = false; if(typeof(bmIsTablerMobile) == 'undefined' || !bmIsTablerMobile()) setCookie('hSepWidth1'+instName, parseInt(hSep1.style.width)); if(typeof(IE7) != 'undefined') IE7.recalc(); };
 	...
 	var omm = function(event)
 	{
		...
		if(_hSepDragging && (typeof(bmIsTablerMobile) == 'undefined' || !bmIsTablerMobile()))
		{
			...
			_hSepApplyWidths(hSep1, hSep2, hSepSep, sepParent, width1, sepWidth);
	...
	var or = function(event)
	{
		...
		if(typeof(bmIsTablerMobile) != 'undefined' && bmIsTablerMobile())
		{
			hSep1.style.cssText = hSep2.style.cssText = hSepSep.style.cssText = '';
			return;
		}
		...
		else if(instName == 'notes')
			initialWidth1 = Math.floor((parentWidth - sepWidth) / 2);
		else
			initialWidth1 = max(320, Math.floor(parentWidth/2.7)-5);

		_hSepApplyWidths(hSep1, hSep2, hSepSep, sepParent, initialWidth1, sepWidth);
...
{
	if(!disableHide || really==true)
		hideHeaderPopup('searchPopup');
}
function hideHeaderPopup(name)
{
	var box = EBID(name);
	if(!box)
		return;
...
{
	var box = EBID(name);
	var right = getElementMetrics(document.body, 'w') - getElementMetrics(elem, 'x') - getElementMetrics(elem, 'w') - 14;
	box.style.right = Math.max(8, right) + 'px';
	box.style.top = (getElementMetrics(elem, 'y') + getElementMetrics(elem, 'h') + 8) + 'px';
	box.style.left = 'auto';
	box.style.display = '';
...
function toggleGroup(id, groupID)
{
	...
	if(groupItem.style.display == '')
	{
		...
		if(groupItemImg.tagName == 'IMG')
			groupItemImg.src = groupItemImg.src.replace(/contract/, 'expand');
		else if(groupItemImg.classList.contains('ti-chevron-down'))
			groupItemImg.classList.replace('ti-chevron-down', 'ti-chevron-right');
	...
	else
	{
		...
		else if(groupItemImg.classList.contains('ti-chevron-right'))
			groupItemImg.classList.replace('ti-chevron-right', 'ti-chevron-down');
...
{
	document.onmouseup = hideUserMenu;
	showHeaderPopup('userMenu', elem);
}
