...
function addrImportDialog(sid)
{
	var book = EBID('importAddressbook'),
		url = 'organizer.addressbook.php?action=importDialog&type=' + EBID('importType').value + '&encoding=' + EBID('importEncoding').value;
	if(book)
...
function todoActionIconButton(iconClass, title, onclick, extraClass)
{
	var a = document.createElement('a');
	a.href = '#';
	a.className = 'btn btn-icon' + (extraClass ? ' ' + extraClass : '');
	a.title = title;
...
function reloadTodoLists(xml, scrollDown)
{
	...
	else
	{
		...
		var editTitle = container.getAttribute('data-edit-title') || '';
		var leaveTitle = container.getAttribute('data-leave-title') || '';
		var deleteTitle = container.getAttribute('data-delete-title') || '';
		var actionsLabel = container.getAttribute('data-actions-label') || '';
		var editIcon = container.getAttribute('data-edit-icon') || 'ti ti-pencil icon';
		...
		for(var i=0; i<items.length; i++)
		{
			var item = items.item(i),
				...
				canEdit = todoXmlVal(item, 'can_edit') == '1';
			...
			actions.className = 'btn-group btn-group-sm bm-organizer-tasklist-actions';
			...
			if(canEdit)
			{
				actions.appendChild(todoActionIconButton(editIcon, editTitle, (function(id) {
					return function(e) {
						if(e) { e.preventDefault(); e.stopPropagation(); }
						return organizerOpenOverlay(bmAppendSession('organizer.todo.php?action=editList&id='+id), editTitle, 440, 220);
					};
				})(itemID)));
			}
			else
				actions.appendChild(todoDisabledIconButton(editIcon, editTitle));
			...
			if(canShare)
			{
				if(canShareItem)
				{
					...
					actions.appendChild(todoDisabledIconButton(shareIcon, shareTitle));
			}

			if(canDelete)
			{
				actions.appendChild(todoActionIconButton(trashIcon, deleteTitle, (function(id) {
					return function(e) {
						if(e) { e.preventDefault(); e.stopPropagation(); }
						deleteTaskList(id);
						return false;
			...
			else if(canLeave)
			{
				...
				actions.appendChild(todoDisabledIconButton(trashIcon, deleteTitle));
...
{
	var wholeDay = EBID('calendarWholeDayBody'),
	...
	var isWeekGrid = day.className.indexOf('bm-organizer-week-grid') !== -1;

	var or = function()
	{
		var parentHeight = cParent.clientHeight;
		if(parentHeight < 50)
		{
			var page = cParent.closest ? cParent.closest('.bm-organizer-page') : null;
			if(page)
			{
				var header = page.querySelector('.bm-organizer-header');
				var footer = page.querySelector('.bm-organizer-footer');
				parentHeight = page.clientHeight
					- (header ? header.offsetHeight : 0)
					- (footer ? footer.offsetHeight : 0);
			}
			else
				parentHeight = getElementMetrics(cParent, 'h');
		}

		var wholeDayHeight = wholeDay ? wholeDay.offsetHeight : 0;

		if(isWeekGrid)
			day.style.height = '';
		else
			day.style.height = Math.max(parentHeight - wholeDayHeight, 120) + 'px';
...
function calendarMonthSizer()
{
	var cParent = EBID('calendarContainer');
	if(!cParent)
		return;

	var table = cParent.querySelector('.bm-organizer-month-table');
	if(!table)
		return;

	var thead = table.querySelector('thead');
	var rows = table.querySelectorAll('tbody tr.bm-organizer-month-week');
	if(!rows.length)
		return;

	var or = function()
	{
		var containerHeight = getElementMetrics(cParent, 'h');
		var headerHeight = thead ? getElementMetrics(thead, 'h') : 0;
		var rowHeight = Math.max(Math.floor((containerHeight - headerHeight) / rows.length), 48);
...
function checkCalendarDateForm(form)
{
	...
	// Make sure the hidden SmartyDateTime fields reflect the current values of
	// the HTML5 date/time inputs (in case an onchange event was missed).
	if(typeof syncSmartyDate === 'function')
	{
...
// Segmented control: called from both radio buttons. Whichever one triggered
// the change reflects the current state via #wholeDay1's `.checked`.
function toggleWholeDay(el)
{
	...
	// Keep the hidden Smarty fields consistent (zero out the H/M when whole day)
	if(typeof syncSmartyDate === 'function')
	{
		syncSmartyDate('startdate');
		syncSmartyDate('enddate');
...
function _calendarInitMonthRange()
{
	...
	function onMouseUp()
	{
		...
		// Ein Tag oder mehrere Tage – in beiden Fällen wird der "Termin
		// anlegen"-Dialog mit einer Ganztages-Auswahl geöffnet. Bei einem
		// einzelnen Klick war das früher ein No-Op, sodass ein Klick abseits
		// der Tages-Zahl gar nichts tat.
		var startTs = cellList[min].ts,
			days = (max - min) + 1,
			// Ganztages-Termine werden hier inklusiv gezählt: 1 Tag Auswahl →
			// Beginn = Ende = selektierter Tag; 2 Tage Auswahl → Beginn = Tag 1,
			// Ende = Tag 2 (nicht Tag 3). Deshalb (days-1)*24 statt days*24.
			durationHours = Math.max(0, days - 1) * 24,
			durationMinutes = 0;

		// WICHTIG: Server berechnet enddate als $startTime + duration und fällt
		// bei fehlendem `time`-Parameter auf time() (=jetzt) zurück – nicht auf
		// `date`. Ohne `time=startTs` würde ein Klick auf 9.-10. deshalb heute+48h
		// als Ende ergeben (z.B. 14.-16.). Wir schicken den Tages-Timestamp mit,
		// damit start- und end-Datum vom gewählten Datum aus berechnet werden.
		var url = _calendarBuildAddDateUrl(startTs, startTs, durationHours, durationMinutes, true);
