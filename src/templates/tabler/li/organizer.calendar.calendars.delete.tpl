...
<form method="post" target="_top" class="bm-organizer-collection-form" action="{sessionurl file='organizer.calendar.php' params="action=calendars&do=delete&id={$calendarItem.id}&csrf_token={$csrfToken}"}">
	{csrffield}
	<div class="modal-body">
		<p class="mb-1">{lng p="realdel"}</p>
		<p class="bm-organizer-collection-name mb-0">{text value=$calendarItem.title}</p>
	</div>
