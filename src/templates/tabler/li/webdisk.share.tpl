...
<form action="webdisk.php?action=saveShareSettings&folder={$folderID}&id={$id}&sid={$sid}" method="post">
	<table class="listTable">
		...
		<tr>
			...
			<td class="listTableRight"><label class="form-check mb-0"><input type="checkbox" class="form-check-input" name="shareFolder" id="shareFolder" {if $folderShared}checked="checked" {/if} /></label></td>
		</tr>
		<tr>
