...
<form name="f1" method="post" action="{sessionurl file='prefs.php' params='action=extpop3&do=action'}">
	{csrffield}
...
<table class="bigTable table table-vcenter table-hover bm-prefs-table">
	<tr>
		...
		<th>
			<a href="{sessionurl file='prefs.php' params="action=extpop3&sort=p_user&order={$sortOrderInv}"}">{lng p="username"}</a>
			{if $sortColumn=='p_user'}<i class="fa {$sortOrder}" aria-hidden="true"></i>{/if}
		</th>
		<th class="bm-prefs-col-host">
			<a href="{sessionurl file='prefs.php' params="action=extpop3&sort=p_host&order={$sortOrderInv}"}">{lng p="host"}</a>
		...
		<th class="bm-prefs-col-lastfetch">
			<a href="{sessionurl file='prefs.php' params="action=extpop3&sort=last_fetch&order={$sortOrderInv}"}">{lng p="lastfetch"}</a>
	...
	<tr>
		...
		<td class="{$class} bm-prefs-col-actions text-end" nowrap="nowrap">
			<div class="btn-group btn-group-sm bm-prefs-row-actions" role="group" aria-label="{lng p="actions"}">
				<a href="{sessionurl file='prefs.php' params="action=extpop3&do=edit&id={$accountID}"}" class="btn btn-outline-secondary btn-icon" title="{lng p="edit"}" aria-label="{lng p="edit"}"><i class="ti ti-pencil icon" aria-hidden="true"></i></a>
				<a onclick="return confirm('{lng p="realdel"}');" href="{sessionurl file='prefs.php' params="action=extpop3&do=delete&id={$accountID}&csrf_token={$csrfToken}"}" class="btn btn-outline-secondary btn-icon text-danger" title="{lng p="delete"}" aria-label="{lng p="delete"}"><i class="ti ti-trash icon" aria-hidden="true"></i></a>
			</div>
		</td>
	</tr>
	{/foreach}
...
<div id="contentFooter" class="contentFooter bm-organizer-footer bm-prefs-footer">
	...
	<div class="right">
		{if $allowAdd}<button type="button" class="btn btn-sm btn-primary" onclick="document.location.href='{sessionurl file='prefs.php' params='action=extpop3&do=add'}';">
