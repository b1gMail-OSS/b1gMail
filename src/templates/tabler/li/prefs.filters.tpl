...
<table class="bigTable table table-vcenter table-hover bm-prefs-table">
	...
	<tr>
		...
		<td class="{if $sortColumn=='title'}listTableTDActive{else}{$class}{/if}" nowrap="nowrap"><a href="prefs.php?sid={$sid}&action=filters&do=edit&id={$filterID}"><i class="ti ti-filter icon icon-sm text-secondary me-1" aria-hidden="true"></i>{text value=$filter.title}</a></td>
		...
		<td class="{if $sortColumn=='orderpos'}listTableTDActive{else}{$class}{/if} bm-prefs-col-orderpos" nowrap="nowrap">
			...
			<div class="btn-group btn-group-sm bm-prefs-row-actions bm-prefs-orderpos-actions ms-1" role="group" aria-label="{lng p="orderpos"}">
				<a href="prefs.php?action=filters&down={$filterID}&sid={$sid}" class="btn btn-outline-secondary btn-icon"><i class="ti ti-arrow-down icon" aria-hidden="true"></i></a>
				<a href="prefs.php?action=filters&up={$filterID}&sid={$sid}" class="btn btn-outline-secondary btn-icon"><i class="ti ti-arrow-up icon" aria-hidden="true"></i></a>
			</div>
		</td>
		...
		<td class="{$class} bm-prefs-col-actions text-end" nowrap="nowrap">
			<div class="btn-group btn-group-sm bm-prefs-row-actions" role="group" aria-label="{lng p="actions"}">
				<a href="prefs.php?action=filters&do=edit&id={$filterID}&sid={$sid}" class="btn btn-outline-secondary btn-icon" title="{lng p="edit"}" aria-label="{lng p="edit"}"><i class="ti ti-pencil icon" aria-hidden="true"></i></a>
				<a onclick="return confirm('{lng p="realdel"}');" href="prefs.php?action=filters&do=delete&id={$filterID}&sid={$sid}" class="btn btn-outline-secondary btn-icon text-danger" title="{lng p="delete"}" aria-label="{lng p="delete"}"><i class="ti ti-trash icon" aria-hidden="true"></i></a>
			</div>
		</td>
	</tr>
	{/foreach}
...
<div id="contentFooter" class="contentFooter bm-organizer-footer bm-prefs-footer">
	...
	<div class="right">
		<button type="button" class="btn btn-sm btn-primary" onclick="document.location.href='prefs.php?action=filters&do=add&sid={$sid}';">
