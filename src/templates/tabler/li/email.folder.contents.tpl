...
<div id="contentHeader">
	<div class="left"{if $templatePrefs.showCheckboxes} style="padding-left:2px;"{/if}>
		...
		<i class="ti {if $folderInfo.type == 'inbox'}ti-inbox{elseif $folderInfo.type == 'outbox'}ti-send{elseif $folderInfo.type == 'drafts'}ti-file-pencil{elseif $folderInfo.type == 'spam'}ti-ban{elseif $folderInfo.type == 'trash'}ti-trash{elseif $folderInfo.type == 'intellifolder'}ti-folder{else}ti-folder{/if}" aria-hidden="true"></i> {$folderInfo.title}
	</div>
	...
	<div class="right bm-mail-header-actions">
		...
		<button type="button" class="btn btn-sm btn-ghost-secondary" onclick="switchPage({$pageNo})" title="{lng p="refresh"}" aria-label="{lng p="refresh"}">
			<i class="ti ti-refresh icon icon-sm me-1" aria-hidden="true"></i>{lng p="refresh"}
		</button>

		{if empty($folderInfo.readonly)}<button type="button" class="btn btn-sm btn-ghost-secondary" onclick="folderViewOptions({$folderID});" title="{lng p="viewoptions"}" aria-label="{lng p="viewoptions"}">
			<i class="ti ti-layout-sidebar-right icon icon-sm me-1" aria-hidden="true"></i>{lng p="viewoptions"}
		</button>{/if}
	</div>
</div>
...
<table class="bigTable" id="mailTable">
	<colgroup>
		{if $templatePrefs.showCheckboxes}<col class="bm-mail-col-check" />{/if}
		<col class="bm-mail-col-icon" />
		<col class="bm-mail-col-from" />
		<col class="bm-mail-col-subject" />
		<col class="bm-mail-col-date" />
		<col class="bm-mail-col-size" />
		<col class="bm-mail-col-actions" />
	</colgroup>
	<thead>
	<tr>
...
<div id="contentFooter" class="contentFooter bm-mail-list-footer">
	<div class="bm-mail-list-footer-row">
		...
		<div class="bm-mail-footer-pagination">
			<span class="bm-mail-footer-pagenav d-none d-md-inline">{pageNav page=$pageNo pages=$pageCount on=" <span class=\"bm-mail-page-current\">[.t]</span> " off=" <a class=\"pageNav\" href=\"javascript:void(0);\" onclick=\"switchPage(.s);\">.t</a> "}</span>
