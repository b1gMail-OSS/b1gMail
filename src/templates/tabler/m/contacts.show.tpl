<div data-role="header" data-position="fixed">
	<a href="contacts.php" data-icon="arrow-l" data-direction="reverse" data-transition="slide">{lng p="contacts"}</a>
	<h1>{$pageTitle}</h1>
</div>
...
<div data-role="content">
	...
	<ul data-role="listview" data-inset="true">
		{if $contact.email}<li><a href="email.php?action=compose&to={$privEmailTo}">{text value=$contact.email}</a></li>{/if}
		...
		{if $contact.work_email}<li><a href="email.php?action=compose&to={$workEmailTo}">{text value=$contact.work_email}</a></li>{/if}
		...
		{if $contact.web}<li><a target="_blank" href="{derefurl url=$contact.web}">{lng p="web"}: {text value=$contact.web}</a></li>{/if}
