{if !$listOnly}<div data-role="header" data-position="fixed">
	<a href="{sessionurl file='email.php' params='action=folders'}" data-icon="arrow-l" data-direction="reverse" data-transition="slide">{lng p="folders"}</a>
	<h1>{$pageTitle}</h1>
	<a href="{sessionurl file='email.php' params='action=compose'}" data-icon="forward" data-iconpos="right" data-dom-cache="false">{lng p="sendmail"}</a>
</div>
...
<div data-role="content">
	...
	{foreach from=$mails item=mail key=mailID}
		<li>
			<a href="{sessionurl file='email.php' params="action=read&id={$mailID}"}" data-transition="slide">
				{if $mail.flags&1}
