	...
	document.addEventListener('DOMContentLoaded', function () {
		...
		function syncPushUi() {
			...
			var pServer = (typeof bmPush.getStatus === 'function'
				? bmPush.getStatus()
				: fetch(typeof bmAppendSession === 'function'
					? bmAppendSession('push-api.php?action=status')
					: ('push-api.php?action=status&sid=' + encodeURIComponent(currentSID)), { credentials: 'same-origin' })
			...
			return Promise.all([pBrowser, pServer]).then(function () {
				// Any of these means "push is on" for the UI (browser permission alone is not enough).
		...
		if (testBtn) {
			testBtn.addEventListener('click', function () {
				...
				function testPushUrl() {
					return typeof bmAppendSession === 'function'
						? bmAppendSession('start.php?action=testPush')
						: ('start.php?action=testPush&sid=' + encodeURIComponent(currentSID));
				}
